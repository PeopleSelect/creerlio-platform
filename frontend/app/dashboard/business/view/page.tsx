...
function BusinessProfileViewPageInner() {
  ...
  const sectionOrder = useMemo(() => {
    ...
    const fallback = ['skills', 'experience', 'projects', 'attachments']
          ...
          <div className="space-y-8">
            ...
            <div className="grid lg:grid-cols-12 gap-6">
              ...
              <div className="lg:col-span-9 space-y-6">
                ...
                {/* Social Links — full-width connect section */}
                {socialLinks.length > 1 && sectionVisibility.social !== false ? (
                  <section id="section-social" className="rounded-2xl border border-white/10 bg-slate-950/40 p-6">
                    ...
                    <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-3">
                      {socialLinks.map((link, idx) => {
                        ...
                        const isWebsite   = pl.includes('website')
                        ...
                        const icon = isLinkedIn ? (
                          ...
                          <svg className="w-5 h-5 shrink-0" fill="currentColor" viewBox="0 0 24 24"><path d="M23.498 6.186a3.016 3.016 0 00-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 00.502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 002.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 002.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/></svg>
                        ) : isFacebook ? (
                          ...
                          <svg className="w-5 h-5 shrink-0" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.748l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
                        ) : isCareers ? (
                          <svg className="w-5 h-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
                        ) : (
                          <svg className="w-5 h-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9"/></svg>
                        )
                        ...
                        return (
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className={`flex items-center gap-3 p-3 rounded-xl border transition-all duration-200 ${colors}`}
                          >
                            {icon}
                            <div className="min-w-0">
                              <div className="font-medium text-sm">{link.platform}</div>
                              <div className="text-xs opacity-60 truncate">{String(link.url).replace(/^https?:\/\//, '').replace(/\/$/, '')}</div>
                            </div>
                            <svg className="w-3.5 h-3.5 ml-auto shrink-0 opacity-40" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"/></svg>
                          </a>
                        )
                      })}
                    </div>
