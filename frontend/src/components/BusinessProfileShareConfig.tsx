/**
 * Business Profile Share Configuration Component
 * 
 * Allows businesses to control what sections and media are visible on their public profile
 */
...
interface BusinessProfileShareConfigProps {
...
export default function BusinessProfileShareConfig({
  businessProfileId,
  userId,
  avatarPath,
  bannerPath,
  introVideoId,
  onConfigChange,
}: BusinessProfileShareConfigProps) {
  ...
  useEffect(() => {
    ...
    async function loadConfig() {
      ...
      if (isSavingRef.current) {
        console.log('[BusinessShareConfig] Skipping load - save in progress')
        setLoading(false)
        return
      }
      ...
      try {
        const { data, error } = await supabase
          .from('business_profile_share_config')
          .select('*')
          .eq('business_profile_id', businessProfileId)
          .maybeSingle()

        if (error && error.code !== 'PGRST116') {
          console.error('Error loading business share config:', error)
        } else if (data) {
          // Only update if not currently saving
          if (!isSavingRef.current) {
            setConfig({
              share_intro: data.share_intro ?? true,
              share_social: data.share_social ?? true,
              share_skills: data.share_skills ?? true,
              share_experience: data.share_experience ?? true,
              share_education: data.share_education ?? true,
              share_referees: data.share_referees ?? true,
              share_projects: data.share_projects ?? true,
              share_attachments: data.share_attachments ?? true,
              share_avatar: data.share_avatar ?? true,
              share_banner: data.share_banner ?? true,
              share_intro_video: data.share_intro_video ?? true,
        ...
        console.error('Exception loading business share config:', error)
      } finally {
        setLoading(false)
      }
    }
  ...
  // Update selected media paths when profile media changes
  useEffect(() => {
  ...
  const updateConfig = async (updates: Partial<ShareConfig>) => {
    ...
    try {
      ...
      console.error('Error saving business share config:', error)
