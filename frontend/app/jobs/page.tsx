...
export default function JobsPage() {
  ...
  const fetchMyApplications = async (uid: string) => {
    try {
      ...
      // Try applications table - it uses talent_profile_id, not user_id
      ...
      // Fallback: try job_applications table if it exists (uses user_id)
      ...
      if (!res2.error) {
        const ids = new Set<string>((res2.data || []).map((r: any) => String(r?.job_id)).filter(Boolean))
        setAppliedJobs(ids)
        return
      }
  ...
  const handleApply = async (jobId: string) => {
    ...
    try {
      ...
      // Validate jobId is valid - Supabase uses UUIDs, not integers
      if (!jobId) {
        console.error('handleApply: jobId is missing', { jobId, jobIdType: typeof jobId })
        throw new Error('Job ID is missing. Please refresh the page and try again.')
      }

      // Validate jobId - Supabase may use UUIDs even if schema says BIGINT
      const jobIdStr = String(jobId).trim()
      if (!jobIdStr) {
        throw new Error('Job ID is missing. Please refresh the page and try again.')
      }

      // Accept both UUID and numeric formats
      // Pass as string - Supabase will handle type conversion if needed
      // RLS policy requires user_id to match auth.uid()
      const payload = { 
        job_id: jobIdStr, // Pass as string - let Supabase handle type conversion
        talent_profile_id: talentProfile.id,
        user_id: userId, // Required by RLS policy: auth.uid() = user_id
        status: 'applied'
      }
