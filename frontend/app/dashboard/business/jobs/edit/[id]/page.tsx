  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        if (!uid) {
          router.push(`/login?redirect=/dashboard/business/jobs/edit/${jobId}`)
          return
        }
        ...
        const businessProfileId = String(row.id)
        setBusinessProfile({
        ...
        if (jobError || !jobData) {
          setErrors({ submit: 'Job not found or you do not have permission to edit it.' })
          if (!cancelled) setIsLoading(false)
          return
        }

        // Verify ownership
        const jobBusinessId = jobData.business_profile_id || jobData.business_id || jobData.company_id
        if (String(jobBusinessId) !== businessProfileId) {
          setErrors({ submit: 'You do not have permission to edit this job.' })
          if (!cancelled) setIsLoading(false)
          return
        }
        ...
        if (!cancelled) {
          setFormData({
            ...
            salary_min: jobData.salary_min ? String(jobData.salary_min) : '',
            salary_max: jobData.salary_max ? String(jobData.salary_max) : '',
        ...
        setErrors({ submit: 'Failed to load job data. Please try again.' })
      } finally {
        if (!cancelled) setIsLoading(false)
      }
    })()
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-900 via-slate-800 to-slate-900">
      ...
      <div className="container mx-auto px-6 py-8 max-w-4xl">
        <h1 className="text-4xl font-bold text-white mb-8">Edit Job Posting</h1>
