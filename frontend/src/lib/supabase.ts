    ...
    if (urlStr.includes('supabase.co/rest/v1/') && urlStr.includes('user_id=eq.')) {
      ...
      if (hasEmailEncoded || hasEmailDecoded) {
        ...
        if (emailMatch && emailMatch[1].includes('@')) {
          console.error(
            '[Supabase Fetch Interceptor] DETECTED: Email used as user_id in URL:',
            emailMatch[1],
            'Full URL:',
            urlStr
          );
          ...
          try {
            ...
            fetch('http://127.0.0.1:7243/ingest/6182f207-3db2-4ea3-b5df-968f1e2a56cc', {
              ...
              body: JSON.stringify({
                location: 'lib/supabase.ts:fetch:interceptor',
                message: 'DETECTED: Email in Supabase URL',
                data: { url: urlStr, decoded, email: emailMatch[1], stack: stack?.split('\n').slice(0, 20).join('\n') },
                timestamp: Date.now(),
  ...
  try {
    ...
    fetch('http://127.0.0.1:7243/ingest/6182f207-3db2-4ea3-b5df-968f1e2a56cc', {
      ...
      body: JSON.stringify({
        ...
        message: 'Fetch interceptor installed',
        data: {},
        timestamp: Date.now(),
        sessionId: 'debug-session',
        runId: 'run1',
                  ...
                  if (column === 'user_id' && typeof val === 'string' && val.includes('@')) {
                    ...
                    try {
                      ...
                      fetch('http://127.0.0.1:7243/ingest/6182f207-3db2-4ea3-b5df-968f1e2a56cc', {
                        ...
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({
                          location: 'lib/supabase.ts:eq:user_id',
                          message: 'DETECTED: Email used as user_id',
                          data: { value: val, table, stack: stack?.split('\n').slice(0, 10).join('\n') },
                          timestamp: Date.now(),
                          sessionId: 'debug-session',
                          runId: 'run1',
                          hypothesisId: 'H4',
