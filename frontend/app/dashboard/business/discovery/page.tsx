          ...
          {filtered.map((t) => (
            <div key={t.talent_profile_id} className="rounded-2xl border border-white/10 bg-white/5 p-5">
              <div className="flex items-start gap-4">
                {/* Anonymous avatar — never shows photo or name initial */}
                ...
                <div className="flex-1 min-w-0">
                  {/* No name shown — identity revealed only after connection accepted */}
                  <div className="text-xs text-slate-500 font-mono mb-1">
                    Anonymous Talent · {t.talent_profile_id.slice(0, 8)}
                  </div>
                  {t.headline ? <div className="font-semibold text-white break-words">{t.headline}</div> : null}
